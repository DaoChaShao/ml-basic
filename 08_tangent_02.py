...
# -*- Coding: UTF-8 -*-
# @Time     :   2025/10/8 21:46
# @Author   :   Shawn
# @Version  :   Version 0.1.0
# @File     :   08_tangent_02.py
# @Desc     :   
...
def main() -> None:
    ...
    index_m = randint(0, len(features) - 1)
    point_m = features[index_m]
    index_n = randint(0, len(features) - 1)
    point_n = features[index_n]

    # Get a tangent line function at the selected point
    tangent_m = tangent_func(model, point_m)
    # Draw the tangent line with the tangent function with the effective range of the model
    y_m = tangent_m(features)
    tangent_n = tangent_func(model, point_n)
    y_n = tangent_n(features)

    # Plot the tangent line and the model function with PySide6
    data_model: DataFrame = DataFrame({"x": features, "y": y_pred})
    data_m: DataFrame = DataFrame({"x": features, "y": y_m})
    data_n: DataFrame = DataFrame({"x": features, "y": y_n})
    line_titles: list[str] = ["Model", f"Tangent@{point_m:.2f}", f"Tangent@{point_n:.2f}"]
    line_dfs: list[DataFrame] = [data_model, data_m, data_n]
